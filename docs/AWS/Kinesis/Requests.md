## Module AWS.Kinesis.Requests

#### `addTagsToStream`

``` purescript
addTagsToStream :: forall eff. Service -> AddTagsToStreamInput -> Aff (exception :: EXCEPTION | eff) NoOutput
```
...
``` purescript
createStream :: forall eff. Service -> CreateStreamInput -> Aff (exception :: EXCEPTION | eff) NoOutput
```
...
``` purescript
decreaseStreamRetentionPeriod :: forall eff. Service -> DecreaseStreamRetentionPeriodInput -> Aff (exception :: EXCEPTION | eff) NoOutput
```
...
``` purescript
deleteStream :: forall eff. Service -> DeleteStreamInput -> Aff (exception :: EXCEPTION | eff) NoOutput
```
...
``` purescript
describeLimits :: forall eff. Service -> DescribeLimitsInput -> Aff (exception :: EXCEPTION | eff) DescribeLimitsOutput
```
...
``` purescript
describeStreamSummary :: forall eff. Service -> DescribeStreamSummaryInput -> Aff (exception :: EXCEPTION | eff) DescribeStreamSummaryOutput
```
...
``` purescript
disableEnhancedMonitoring :: forall eff. Service -> DisableEnhancedMonitoringInput -> Aff (exception :: EXCEPTION | eff) EnhancedMonitoringOutput
```
...
``` purescript
enableEnhancedMonitoring :: forall eff. Service -> EnableEnhancedMonitoringInput -> Aff (exception :: EXCEPTION | eff) EnhancedMonitoringOutput
```
...
``` purescript
increaseStreamRetentionPeriod :: forall eff. Service -> IncreaseStreamRetentionPeriodInput -> Aff (exception :: EXCEPTION | eff) NoOutput
```
...
``` purescript
listShards :: forall eff. Service -> ListShardsInput -> Aff (exception :: EXCEPTION | eff) ListShardsOutput
```
...
``` purescript
mergeShards :: forall eff. Service -> MergeShardsInput -> Aff (exception :: EXCEPTION | eff) NoOutput
```
...
``` purescript
putRecords :: forall eff. Service -> PutRecordsInput -> Aff (exception :: EXCEPTION | eff) PutRecordsOutput
```
...
``` purescript
removeTagsFromStream :: forall eff. Service -> RemoveTagsFromStreamInput -> Aff (exception :: EXCEPTION | eff) NoOutput
```
...
``` purescript
splitShard :: forall eff. Service -> SplitShardInput -> Aff (exception :: EXCEPTION | eff) NoOutput
```
...
``` purescript
startStreamEncryption :: forall eff. Service -> StartStreamEncryptionInput -> Aff (exception :: EXCEPTION | eff) NoOutput
```
...
``` purescript
stopStreamEncryption :: forall eff. Service -> StopStreamEncryptionInput -> Aff (exception :: EXCEPTION | eff) NoOutput
```
...
``` purescript
updateShardCount :: forall eff. Service -> UpdateShardCountInput -> Aff (exception :: EXCEPTION | eff) UpdateShardCountOutput
```
